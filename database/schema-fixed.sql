-- DroneFlow Database Schema - Fixed Version for Supabase
...
-- Create service_requests table with correct column names
CREATE TABLE service_requests (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    request_id VARCHAR(20) UNIQUE NOT NULL,
    
    -- Client Information (matching JavaScript field names exactly)
...
CREATE INDEX idx_service_requests_service_type ON service_requests("serviceType");
...
CREATE INDEX idx_service_requests_preferred_date ON service_requests("preferredDate");
...
-- Policy to allow anyone to insert new requests
CREATE POLICY "Anyone can create service requests" ON service_requests
    FOR INSERT TO anon, authenticated
    WITH CHECK (true);

-- Policy to allow anyone to view requests (for demo purposes)
CREATE POLICY "Anyone can view service requests" ON service_requests
    FOR SELECT TO anon, authenticated
    USING (true);

-- Policy to allow updates (for admin operations)
CREATE POLICY "Anyone can update service requests" ON service_requests
    FOR UPDATE TO anon, authenticated
    USING (true);

-- Policy to allow deletes (for admin operations)
CREATE POLICY "Anyone can delete service requests" ON service_requests
...
-- Success message
SELECT 'DroneFlow database schema has been successfully created!' as message;